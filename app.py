...
with col1:
    ...
    if word_file: st.session_state["uploaded_word"] = word_file
...
with col2:
    ...
    if pdf_file: st.session_state["uploaded_pdf"] = pdf_file
...
if st.button("🚀 開始比對", use_container_width=True):
    word_file = st.session_state.get('uploaded_word')  # 從 session_state 取回
