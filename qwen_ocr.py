...
class QwenOCR:
    ...
    def extract_table(self, page):
        """從 PyMuPDF 頁面對象中提取表格內容
        
        Args:
            page (fitz.Page): PyMuPDF 頁面對象
        ...
        try:
            ...
            # 使用現有方法提取表格
            if self.use_free_api:
                tables = self._extract_tables_free_api(base64_image)
            else:
                tables = self._extract_tables_official_api(base64_image)
                
            # 處理返回結果
            if isinstance(tables, list) and len(tables) > 0:
                return tables[0]  # 返回第一個表格
            elif isinstance(tables, dict) and "tables" in tables:
                return tables["tables"][0] if tables["tables"] else []
            else:
                return []
        except Exception as e:
            st.warning(f"提取表格時出錯: {str(e)}")
            return []
